...
class ResumableUploadHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        print(f"[{timestamp}] 📍 Client: {self.client_address[0]}:{self.client_address[1]}")
    ...
    def do_POST(self):
        ...
        print(f"[{timestamp}] 📍 Client: {self.client_address[0]}:{self.client_address[1]}")
        print(f"[{timestamp}] 📦 Content-Length: {self.headers.get('Content-Length', 'N/A')} bytes")
        print(f"[{timestamp}] 🔧 Content-Type: {self.headers.get('Content-Type', 'N/A')[:50]}...")
        print(f"{'='*60}")
        ...
        try:
            ...
            for part in parts:
                if b'name="chunk"' in part:
                    ...
                    chunk_index = int(part.split(b'\r\n\r\n', 1)[1].rsplit(b'\r\n', 1)[0])
                elif b'name="totalChunks"' in part:
                    total_chunks = int(part.split(b'\r\n\r\n', 1)[1].rsplit(b'\r\n', 1)[0])
            ...
            print(f"[{timestamp}] ✅ Chunk saved! Accumulated size: {current_mb:.2f} MB")

            # Log progress for every 100 chunks or final chunk
            if (chunk_index + 1) % 100 == 0 or chunk_index + 1 >= total_chunks:
                print(f"\n{'='*60}")
                print(f"[{timestamp}] 📊 PROGRESS: {chunk_index + 1}/{total_chunks} ({percent}%)")
