...
class ServerLogger:
    ...
    def log_request(self, method: str, path: str, client_address: tuple, user_agent: str = None):
        """
        ...
        if user_agent:
            msg += f" ({user_agent[:50]}...)" if len(user_agent) > 50 else f" ({user_agent})"
        
        self.logger.info(msg)
    
    def log_upload_progress(self, filename: str, chunk_index: int, total_chunks: int, 
                           current_size: int, processing_time: float):
        """
        Log upload progress.
        """
